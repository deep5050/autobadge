...
var get_badge_text =
    (provider, path, url, alt) => {
      ...
      if (type === 'HTML') {
        return `<a href="${url}"><img alt="${alt}" src="${provider}${
            path}style=${style}"/></a>`;
...
var other_badges =
    (user, repo) => {
      ...
      var hit_count = get_badge_text("http://hits.dwyl.com",
                                     `/${user}/${repo}.svg?`, url, 'Hit Count');
...
var social_badges =
    (user, repo) => {
      ...
      var followers = get_badge_text(shields, `/github/followers/${user}?`,
      ...
      var forks = get_badge_text(
          shields, `/github/forks/${user}/${repo}?`,
          `https://github.com/${user}/${repo}/network/members`, 'Forks');

      // console.log(forks);
      text += make_a_row(forks);

      var stars = get_badge_text(
          shields, `/github/stars/${user}/${repo}?`,
          `https://github.com/${user}/${repo}/stargazers`, 'Stars');
      // console.log(stars);
      text += make_a_row(stars);

      var watchers = get_badge_text(
          shields, `/github/watchers/${user}/${repo}?`,
          `https://github.com/${user}/${repo}/watchers`, 'Watchers');
...
var version_badges =
    (user, repo) => {
      ...
      var version =
          get_badge_text(shields, `/github/v/release/${user}/${repo}?`, url,
                         'Release Version');
      // console.log(version);
...
var ci_cd_badges =
    (user, repo) => {
      ...
      var github_workflow = get_badge_text(
          shields,
          `/github/workflow/status/${user}/${repo}/${
              config.get('github.workflow')}?label=${
              config.get('github.workflow')}&logo=github&`,
          url + `/actions?query=workflow:${config.get('github.workflow')}`,
          'GitHub Workflow Status');

      text += make_a_row(github_workflow);

      if (config.has('ci_cd_provider') &&
          config.get('ci_cd_provider') === 'Travis') {
        var travis = get_badge_text(
            shields, `/travis/${user}/${repo}?label=Build&logo=travis&`, url,
            'Travis Build');
...
var activity_badges =
    (user, repo) => {
      ...
      var commits_per_month =
          get_badge_text(shields, `/github/commit-activity/m/${user}/${repo}?`,
                         url, 'Commits/month');
      // console.log(commits_per_month);
      text += make_a_row(commits_per_month);

      var last_commit = get_badge_text(
          shields, `/github/last-commit/${user}/${repo}?`, url, 'Last Commit');
      // console.log(last_commit);
      text += make_a_row(last_commit);

      var release_date = get_badge_text(
          shields, `/github/release-date/${user}/${repo}?`,
          `https://github.com/${user}/${repo}/releases`, 'Last release date');
      // console.log(release_date);
      text += make_a_row(release_date);

      var contributors = get_badge_text(
          shields, `/github/contributors/${user}/${repo}?`,
...
var license_badges =
    (user, repo) => {
      ...
      var license = get_badge_text(shields, `/github/license/${user}/${repo}?`,
                                   url, 'License');
      // console.log(license);
...
var size_badges =
    (user, repo) => {
      ...
      var repo_size = get_badge_text(
          shields, `/github/repo-size/${user}/${repo}?`, url, 'Repo Size');
...
var analysis_badges =
    (user, repo) => {
      ...
      var languages_count =
          get_badge_text(shields, `/github/languages/count/${user}/${repo}?`,
                         url, 'Language Count');
      // console.log(languages_count);
      text += make_a_row(languages_count);

      var top_language =
          get_badge_text(shields, `/github/languages/top/${user}/${repo}?`, url,
                         'Top Language');
      // console.log(top_language);
      text += make_a_row(top_language);

      if (config.has('analysis_provider') &&
          config.get('analysis_provider') === 'CodeClimate') {
        var codeclimate_maintain_percent = get_badge_text(
            shields, `/codeclimate/maintainability-percentage/${user}/${repo}?`,
            url, 'Code Climate maintainability');
        // console.log(codeclimate_maintain_percent);
        text += make_a_row(codeclimate_maintain_percent);

        var codeclimate_issues =
            get_badge_text(shields, `/codeclimate/issues/${user}/${repo}?`, url,
                           "Code Climate Issues");
      ...
      if (config.has('analysis_provider') &&
        ...
        if (config.has('codacy.project_id') &&
            config.get('codacy.project_id') !== "") {
          var codacy_project_id = config.get('codacy.project_id');
          var codacy_grade =
              get_badge_text(shields, `/codacy/grade/${codacy_project_id}?`,
                             url, 'Codacy Grade');
...
var issues_badges =
    (user, repo) => {
      ...
      var issue_raw = get_badge_text(
          shields, `/github/issues-raw/${user}/${repo}?`, url, "Github Issues");
      // console.log(issues_raw);
      text += make_a_row(issue_raw);

      var issues_closed = get_badge_text(
          shields, `/github/issues-closed/${user}/${repo}?`,
      ...
      var pr_raw = get_badge_text(
          shields, `/github/issues-pr-raw/${user}/${repo}?`,
          `https://github.com/${user}/${repo}/pulls`, "Github open PRs");
      // console.log(pr_raw);
      text += make_a_row(pr_raw);

      var pr_closed = get_badge_text(
          shields, `/github/issues-pr-closed/${user}/${repo}?`,
...
const interactive_mode =
    async () => {
  var user =
      await qoa.input({query : 'Type your github username:', handle : 'user'});
  config.set(user);
  ...
  if (config.has('analysis_provider') &&
    ...
    var project_id = await qoa.secure(
        {query : 'Your Codacy Project ID:', handle : 'project_id'});
